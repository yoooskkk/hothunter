...
> **API Key / Secret 通过 Freqtrade 原生 `FREQTRADE__` 环境变量注入，不写入配置文件。**

Freqtrade 支持 `FREQTRADE__SECTION__KEY=value` 语法，使用双下划线 `__` 作为路径分隔符，可覆盖任意配置字段：
...
# FREQTRADE__EXCHANGE__KEY=your_binance_api_key
# FREQTRADE__EXCHANGE__SECRET=your_binance_api_secret
# FREQTRADE__TELEGRAM__TOKEN=your_bot_token
# FREQTRADE__TELEGRAM__CHAT_ID=your_chat_id
```

> 配置文件中的占位符 (`"YOUR_TELEGRAM_BOT_TOKEN"`) 会被环境变量自动覆盖。无需在 config 文件中使用 `${VAR}` 语法。
